...
import { X, Battery, Gauge, MapPin, Bed, Activity, Clock, Navigation, AlertTriangle, ArrowLeft, Sparkles } from 'lucide-react';

const Sidebar = ({ item, onClose, incidents, routeMetrics, vehicles, stations, onSelectVehicle }) => {
    if (!item) return null;

    // Check if we can go back to a station (only for Available vehicles that are at their station)
    ...
    return (
        <div className="glass-panel animate-slide-in" style={{
            ...
            onDragOver={(e) => {
                // Allow dragging over the sidebar when viewing a station
                    ...
                    {item.type === 'Station' ? (
                        ...
                        <div style={{
                            ...
                            onDrop={(e) => {
                                ...
                                // Get the vehicle data from the drag event
                        ...
                        /* Button for Vehicles and Incidents */
                        <button style={{
                            width: '100%',
                            padding: '12px',
                            background: '#3b82f6',
...
const VehicleDetails = ({ vehicle, incidents, routeMetrics, stations }) => {
    // For Available vehicles, use the parent station's mockRouting data
    const parentStation = vehicle.stationId && stations
        ? stations.find(s => s.id === vehicle.stationId)
        : null;

    const useStationRouting = vehicle.status === 'Available' && parentStation?.mockRouting;

    // defaults if no route active
    ...
    if (!isAvailable) {
        if (isReturning) {
            ...
            // Add refueling time if fuel is critically low
            if (vehicle.fuel < 10) {
            ...
            // Try to get mission details if available
    ...
    // Total Estimated Time to Scene = Ready Time + Drive Time + Traffic
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '16px' }}>
                ...
                {/* AI Explanation - Only show for non-available vehicles */}
                {!isAvailable && (
            ...
            {/* Fuel Status Card - Moved Below ETR */}
            <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '16px' }}>
                ...
                vehicle.mission && (
                    <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '16px' }}>
                        ...
                        <div style={{ position: 'relative', paddingLeft: '12px' }}>
                            ...
                            {[...vehicle.mission.timeline].reverse().map((event, idx) => {
                                ...
                                return (
                                    <div key={idx} style={{ marginBottom: '24px', position: 'relative', paddingLeft: '24px' }}>
                                        ...
                                        <div style={{
                                            ...
                                            left: isLatest ? '0px' : '2px', // adjustment for size diff
                                            boxShadow: isLatest ? '0 0 12px rgba(16, 185, 129, 0.6)' : 'none',
                                            zIndex: 1,
                                            border: '2px solid #1e293b' // dark border to separate from line
                                        }} />
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                                            ...
                                            {/* Show keyword for dispatched events */}
                                            {event.event.includes('Dispatched:') && (
                                                <span style={{ fontSize: '0.85rem', color: '#94a3b8' }}>
                                                    Keyword: {event.event.replace('Dispatched: ', '')}
                                                </span>
                                            )}
                                            {/* Dynamic subtitle for other events */}
                                            {event.event === 'Arrived at Scene' && (
                                                <span style={{ fontSize: '0.8rem', color: '#64748b' }}>Friedrich-Ebert-Straße 82</span>
                                            )}
                                            {event.event.includes('Hospital') && (
                                                <span style={{ fontSize: '0.8rem', color: '#64748b' }}>Klinikum Ingolstadt (ER)</span>
                                            )}
                                        </div>
                                    </div>
                                );
...
const HospitalDetails = ({ hospital }) => {
    ...
    // Use dynamic admissions from simulation
...
const StationDetails = ({ station, vehicles, onSelectVehicle, routeMetrics }) => {
    // Filter vehicles at this station
    // Filter vehicles at this station (only show Available ones as requested)
    const stationVehicles = vehicles.filter(v => v.stationId === station.id && v.status === 'Available');
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {/* Estimated Time to Scene for Station */}
            <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '16px' }}>
                <div style={{ marginBottom: '16px', fontSize: '0.75rem', fontWeight: 700, color: '#cbd5e1', textTransform: 'uppercase', letterSpacing: '1px' }}>
                    ESTIMATED TIME TO SCENE
                </div>
                        ...
                        stationVehicles.map(v => (
                            <div
                                ...
                                onDragStart={(e) => {
                                    ...
                                    // Create a much more visible drag image
                                    ...
                                    dragImage.style.backgroundColor = '#0f172a'; // Very dark, solid color
                                    dragImage.style.border = '3px solid #60a5fa'; // Bright blue
                                    dragImage.style.borderRadius = '10px';
                                    dragImage.style.boxShadow = '0 20px 60px rgba(0,0,0,0.9)';
                                    dragImage.style.fontFamily = 'system-ui, -apple-system, sans-serif';

                                    // Create inner content with maximum contrast
                                    dragImage.innerHTML = `
                                        <div style="display: flex; justify-content: space-between; align-items: center;">
                                            <div>
                                                <div style="font-weight: 700; font-size: 16px; color: #ffffff; margin-bottom: 4px;">${v.subtype}</div>
                                                <div style="font-size: 13px; color: #cbd5e1;">${v.name}</div>
                                            </div>
                                            <div style="background: #166534; color: #86efac; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: 700;">
                                                Available
                                            </div>
                                        </div>
                                    `;
